...
namespace SistemaGestionOrquesta.Controllers
{
    ...
    public class EstudianteController : ControllerBase
    {
        ...
        {
            var lista = estudianteService.Get();
            return Ok(lista);
        }
        ...
        public IActionResult GetEstudiantesByCurso(int idCurso)
        {
            try
            {
                var resultado = estudianteService.GetEstudiantesActivosByCurso(idCurso);
                return Ok(resultado);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetCursosByEstudiante(Guid idEstudiante)
        {
            try
            {
                var resultado = await estudianteService.GetCursosByEstudiante(idEstudiante);
                return Ok(resultado);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> DeleteCursoEstudiante(Guid id, int Idcurso)
        {
            try
            {
                var result = await estudianteService.EliminarCursoEstudiante(id, Idcurso);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> DarDeAltaEnCurso(Guid id, int Idcurso)
        {
            try
            {
                var result = await estudianteService.InscribirEstudianteCurso(id, Idcurso);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> EditCurso(Guid id, int cursoNuevo, int cursoViejo)
        {
            ...
            try
            {
                await estudianteService.CambiarEstudianteDeCurso(id, cursoNuevo, cursoViejo);
                return Ok(new { message = "Curso cambiado exitosamente" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { error = ex.Message });
